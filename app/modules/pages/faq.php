...
echo '<b>2</b>. Добавлять свои фотографии в галерею и анкету (Вес картинки не должен превышать '.formatsize($config['filesize']).')<br />';
echo '<b>3</b>. Изменять репутацию другим  пользователям сайта положительными или отрицательным голосом (При активе '.points($config['editratingpoint']).')<br />';
...
echo '<b>6</b>. Общаться по внутренней почте сайта с пользователями сайта, писать под своим ником в гостевой, комментариях, форуме<br />';
...
if (!empty($config['editratingpoint'])) {
    echo '<b>'.points($config['editratingpoint']).'</b> - изменение репутации пользователям<br />';
}
