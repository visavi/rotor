...
echo '<b>2</b>. Добавлять свои фотографии в галерею и анкету (Вес картинки не должен превышать '.formatsize(App::setting('filesize')).')<br />';
echo '<b>3</b>. Изменять репутацию другим  пользователям сайта положительными или отрицательным голосом (При активе '.points(App::setting('editratingpoint')).')<br />';
...
echo '<b>7</b>. Играть в игры на деньги. Копить их в банке, брать кредит, перечислять, принимать, покупать виртуальные товары<br />';
echo '<b>8</b>. Участвовать в голосованиях на различные темы<br />';
echo '<b>9</b>. Добавлять свои объявления, на определенный срок (Купля,продажа,вакансии,услуги,обмен и т.д.)<br />';
echo '<b>10</b>. Изменять под себя настройки сайта, в частности количество показываемых сообщений в любых сервисах (гостевая, форум и пр.) Чем можно снизить трафик передаваемой информации<br />';

echo '<br />При наборе определенного количества актива, пользователю открываются новые возможности:<br />';

if (!empty(App::setting('addofferspoint'))) {
    echo '<b>'.points(App::setting('addofferspoint')).'</b> - создание тем в "Предложенияx и проблемах"<br />';
}
if (!empty(App::setting('creditpoint'))) {
    echo '<b>'.points(App::setting('creditpoint')).'</b> - получение кредита в банке<br />';
}
if (!empty(App::setting('forumloadpoints'))) {
    echo '<b>'.points(App::setting('forumloadpoints')).'</b> - Прикрепление файлов в форуме<br />';
}
if (!empty(App::setting('sendmoneypoint'))) {
    echo '<b>'.points(App::setting('sendmoneypoint')).'</b> - перечисление игровых денег<br />';
}
if (!empty(App::setting('editratingpoint'))) {
    echo '<b>'.points(App::setting('editratingpoint')).'</b> - изменение репутации пользователям<br />';
}
if (!empty(App::setting('editforumpoint'))) {
    echo '<b>'.points(App::setting('editforumpoint')).'</b> - изменение и закрытие своих тем на форуме<br />';
}
if (!empty(App::setting('advertpoint'))) {
    echo '<b>'.points(App::setting('advertpoint')).'</b> - исчезает вся реклама на главной странице сайта<br />';
}
echo '<br />';
