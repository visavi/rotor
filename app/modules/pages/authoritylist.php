...
############################################################################################
    case 'index':
        ...
        $page = App::paginate(Setting::get('avtorlist'), $total);

        if ($total > 0) {

            $queryusers = DB::run() -> query("SELECT * FROM `users` ORDER BY `rating` DESC, `login` ASC LIMIT ".$page['offset'].", ".Setting::get('avtorlist').";");
    ...
    case 'search':
        ...
        if (!empty($uz)) {
            ...
            if (!empty($queryuser)) {
                ...
                if (!empty($rat)) {
                    $end = ceil($rat / Setting::get('avtorlist'));
