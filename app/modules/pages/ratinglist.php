...
############################################################################################
    case 'index':
        ...
        if ($total > 0) {
            ...
            $queryusers = DB::select("SELECT * FROM `users` ORDER BY `money` DESC, `login` ASC LIMIT ".$page['offset'].", ".setting('userlist').";");
    ...
    case 'search':
        ...
        if (!empty($uz)) {
            ...
            if (!empty($queryuser)) {
                $queryrating = DB::select("SELECT `login` FROM `users` ORDER BY `money` DESC, `login` ASC;");
