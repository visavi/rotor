        ...
        if (count($arrfind) > 0) {
            ...
            if ($wheres == 'title') {
                ...
                if ($total > 0) {
                    ...
                    $queryblog = DB::run() -> query("SELECT `blogs`.*, `category_id`, `name` FROM `blogs` LEFT JOIN `catsblog` ON `blogs`.`category_id`=`catsblog`.`id` WHERE blogs.`id` IN (".$result.") ORDER BY `time` DESC LIMIT ".$page['offset'].", ".Setting::get('blogpost').";");
