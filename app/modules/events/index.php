...
case 'index':
    ...
    $page = App::paginate(Setting::get('postevents'), $total);

	//Setting::get('description') =  'Список событий (Стр. '.$page['current'].')';

	if ($total > 0) {

		$queryevents = DB::run() -> query("SELECT * FROM `events` ORDER BY `time` DESC LIMIT ".$page['offset'].", ".Setting::get('postevents').";");
...
case 'comments':
	...
	if (!empty($dataevent)) {
        ...
        $page = App::paginate(Setting::get('postevents'), $total);
        //Setting::get('description') =  'Комментарии - '.$dataevent['title'].' (Стр. '.$page['current'].')';
		...
		if ($total > 0) {
			...
			$querycomm = DB::run() -> query("SELECT * FROM `comments` WHERE relate_type=? AND `relate_id`=? ORDER BY `time` ASC LIMIT ".$page['offset'].", ".Setting::get('postevents').";", ['event', $id]);
...
case 'end':
	...
	if (!empty($query)) {
		...
		$end = ceil($total_comments / Setting::get('postevents'));
