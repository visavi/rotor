...
DB::run() -> query("DELETE FROM `online` WHERE `updated_at`<?;", [SITETIME - setting('timeonline')]);
...
if (is_user()) {
    ...
    if (empty($queryonline)) {
        ...
        DB::run() -> query("UPDATE `online` SET `ip`=?, `brow`=?, `updated_at`=?, `user_id`=? WHERE `id`=? LIMIT 1;", [getClientIp(), getUserAgent(), SITETIME, getUserId(), $queryonline]);
    }
} else {
    $queryonline = DB::run() -> querySingle("SELECT `id` FROM `online` WHERE `ip`=? ORDER BY user_id IS NULL desc LIMIT 1;", [getClientIp()]);
    if (empty($queryonline)) {
        DB::run() -> query("INSERT INTO `online` (`ip`, `brow`, `updated_at`) VALUES (?, ?, ?);", [getClientIp(), getUserAgent(), SITETIME]);
        $newhost = 1;
    } else {
        DB::run() -> query("UPDATE `online` SET `brow`=?, `updated_at`=?, user_id=? WHERE `id`=? LIMIT 1;", [getUserAgent(), SITETIME, null, $queryonline]);
    }
}
// -----------------------------------------------------------//
