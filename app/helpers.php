...
function sendNotify(string $text, string $pageUrl, string $pageName)
{
    ...
    if (! empty($matches[1])) {
        ...
        foreach ($usersAnswer as $login) {
            ...
            if ($user && $user->notify) {
                $user->sendMessage(null, 'Пользователь ' . (getUser() ? '@' .getUser('login') : setting('guestsuser')) . ' упомянул вас на странице [url=' . $pageUrl . ']' . $pageName . '[/url]' . PHP_EOL . 'Текст сообщения: ' . $text);
            }
        }
    }
}
