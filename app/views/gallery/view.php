<?php if (!empty($photo)): ?>
    ...
    <?php
    $links = array(
        array('url' => '/admin/gallery?act=edit&amp;start='.$start.'&amp;gid='.$photo['id'], 'label' => 'Редактировать', 'show' => is_admin()),
        array('url' => '/admin/gallery?act=del&amp;del='.$photo['id'].'&amp;start='.$start.'&amp;uid='.$_SESSION['token'], 'label' => 'Удалить', 'params' => array('onclick' => "return confirm('Вы подтверждаете удаление изображения?')"), 'show' => is_admin()),
        array('url' => '/gallery?act=edit&amp;gid='.$photo['id'].'&amp;start='.$start, 'label' => 'Редактировать', 'show' => (($photo['user'] == App::getUsername()) && !is_admin())),
        array('url' => '/gallery?act=delphoto&amp;gid='.$photo['id'].'&amp;start='.$start.'&amp;uid='.$_SESSION['token'], 'label' => 'Удалить', 'params' => array('onclick' => "return confirm('Вы подтверждаете удаление изображения?')"), 'show' => (($photo['user'] == App::getUsername()) && !is_admin())),
    );
