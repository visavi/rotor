		...
		if (!empty($downs['downs_active']) || $downs['downs_user'] == $log) {
			...
			if (!empty($downs['downs_link']) && file_exists(BASEDIR.'/load/files/'.$folder.$downs['downs_link'])) {
				...
				if (is_user()) {
					...
					echo '<input name="text" size="40" value="'.$config['home'].'/load/files/'.$folder.$downs['downs_link'].'" /><br />';
				}
	...
	if (is_user() || $provkod == $_SESSION['protect']) {
		...
		if (!empty($downs)) {
			if (!empty($downs['downs_active'])) {
				...
				if (file_exists('files/'.$folder.$downs['downs_link'])) {
					...
					redirect("files/".$folder.$downs['downs_link']);
				} else {
