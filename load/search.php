...
if (is_user()) {
	...
	############################################################################################
		case "index":
			...
			echo '<div class="form"><form action="search.php?act=search" method="get">';
			...
			if (utf_strlen($find) >= 3 && utf_strlen($find) <= 50) {
				...
				if ($wheres == 'title') {
					...
					if ($total > 0) {
						...
						$querydown = DB::run() -> query("SELECT `downs`.*, `cats_name` FROM `downs` LEFT JOIN `cats` ON `downs`.`downs_cats_id`=`cats`.`cats_id` WHERE `downs_id` IN (".$result.") ORDER BY `downs_time` DESC LIMIT ".$start.", ".$config['downlist'].";");
						...
						page_strnavigation('search.php?act=search&amp;find='.urlencode($find).'&amp;type='.$type.'&amp;where='.$where.'&amp;', $config['downlist'], $start, $total);
					} else {
				...
				if ($wheres == 'text') {
					...
					if ($total > 0) {
						...
						while ($data = $querydown -> fetch()) {
							...
							echo 'Категория: <a href="down.php?cid='.$data['downs_cats_id'].'">'.$data['cats_name'].'</a><br />';
							echo 'Добавил: '.profile($data['downs_user']).' ('.date_fixed($data['downs_time']).')</div>';
						}
