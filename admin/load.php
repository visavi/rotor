											...
											if (filesize(BASEDIR.'/load/loader/'.$file) > 0 && filesize(BASEDIR.'/load/loader/'.$file) <= $config['fileupload']) {
												...
												if (empty($downlink)) {
													...
													if (file_exists(BASEDIR.'/load/loader/'.$file.'.JPG')) {
														rename(BASEDIR.'/load/loader/'.$file.'.JPG', BASEDIR.'/load/screen/'.$folder.$filename.'.jpg');
														$screen = $filename.'.jpg';
													} elseif (file_exists(BASEDIR.'/load/loader/'.$file.'.GIF')) {
														rename(BASEDIR.'/load/loader/'.$file.'.GIF', BASEDIR.'/load/screen/'.$folder.$filename.'.gif');
														$screen = $filename.'.gif';
													} else {
														$screen = '';
													}

													rename(BASEDIR.'/load/loader/'.$file, BASEDIR.'/load/files/'.$folder.$filename);
					...
					if (empty($catParent) || $parent == 0) {
						...
						if (empty($folder) && ! empty($cat['folder'])) {
							...
							foreach ($downs as $down) {
								...
								if (! empty($down['downs_link']) && file_exists(BASEDIR.'/load/files/'.$cat['folder'].'/'.$down['downs_link'])) {
									rename(BASEDIR.'/load/files/'.$cat['folder'].'/'.$down['downs_link'], BASEDIR.'/load/files/'.$down['downs_link']);
								}

								if (! empty($down['downs_screen']) && file_exists(BASEDIR.'/load/screen/'.$cat['folder'].'/'.$down['downs_screen'])) {

									rename(BASEDIR.'/load/screen/'.$cat['folder'].'/'.$down['downs_screen'], BASEDIR.'/load/screen/'.$down['downs_screen']);
									unlink_image('load/screen/'.$cat['folder'], $down['downs_screen']);
								}
							}

							removeDir(BASEDIR.'/load/files/'.$cat['folder']);
							removeDir(BASEDIR.'/load/screen/'.$cat['folder']);
							$renameDir = true;
						}

						if (! empty($folder) && ! file_exists(BASEDIR.'/load/files/'.$folder) && $cat['folder'] != $folder) {

							if (! empty($cat['folder']) && file_exists(BASEDIR.'/load/files/'.$cat['folder'])){
								rename(BASEDIR.'/load/files/'.$cat['folder'], BASEDIR.'/load/files/'.$folder);
								rename(BASEDIR.'/load/screen/'.$cat['folder'], BASEDIR.'/load/screen/'.$folder);
							} else {
								$old = umask(0);
								mkdir(BASEDIR.'/load/files/'.$folder, 0777, true);
								mkdir(BASEDIR.'/load/screen/'.$folder, 0777, true);
								umask($old);

								foreach ($downs as $down) {

									if (! empty($down['downs_link']) && file_exists(BASEDIR.'/load/files/'.$down['downs_link'])) {

										rename(BASEDIR.'/load/files/'.$down['downs_link'], BASEDIR.'/load/files/'.$folder.'/'.$down['downs_link']);
									}

									if (! empty($down['downs_screen']) && file_exists(BASEDIR.'/load/screen/'.$down['downs_screen'])) {

										rename(BASEDIR.'/load/screen/'.$down['downs_screen'], BASEDIR.'/load/screen/'.$folder.'/'.$down['downs_screen']);
										unlink_image('load/screen/', $down['downs_screen']);
									}
								}
											...
											if (empty($downtitle)) {
												...
												if (!empty($loadfile) && $loadfile != $new['downs_link'] && file_exists(BASEDIR.'/load/files/'.$folder.$new['downs_link'])) {
													...
													if ($oldext == $newext) {
														...
														rename(BASEDIR.'/load/files/'.$folder.$new['downs_link'], BASEDIR.'/load/files/'.$folder.$loadfile);
														...
														if (!empty($new['downs_screen']) && file_exists(BASEDIR.'/load/screen/'.$folder.$new['downs_screen'])) {
															...
															rename(BASEDIR.'/load/screen/'.$folder.$new['downs_screen'], BASEDIR.'/load/screen/'.$folder.$screen);
	...
	if ($uid == $_SESSION['token']) {
		...
		if (! empty($catsTo)) {
			...
			if (!empty($downFrom)) {
				...
				rename(BASEDIR.'/load/files/'.$folderFrom.$downFrom['downs_link'], BASEDIR.'/load/files/'.$folderTo.$downFrom['downs_link']);
