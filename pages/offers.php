...
############################################################################################
	case 'index':
		...
		if (is_admin(array(101, 102))) {
			echo ' / <a href="/admin/offers.php?type='.$type.'&amp;start='.$start.'">Управление</a>';
		}
		...
		if ($order == 'offers_votes') {
			...
			echo '<a href="offers.php?type='.$type.'&amp;sort=vote">Голоса</a> / ';
		}

		if ($order == 'offers_time') {
			echo '<b>Дата</b> / ';
		} else {
			echo '<a href="offers.php?type='.$type.'&amp;sort=time">Дата</a> / ';
		}

		if ($order == 'offers_status') {
			echo '<b>Статус</b> / ';
		} else {
			echo '<a href="offers.php?type='.$type.'&amp;sort=stat">Статус</a> / ';
		}

		if ($order == 'offers_comments') {
			echo '<b>Комментарии</b>';
		} else {
			echo '<a href="offers.php?type='.$type.'&amp;sort=comm">Комментарии</a>';
		}
		...
		if ($total > 0) {
			...
			while ($data = $queryoffers -> fetch()) {
				...
				echo '<a href="offers.php?act=comments&amp;id='.$data['offers_id'].'">Комментарии</a> ('.$data['offers_comments'].') ';
				echo '<a href="offers.php?act=end&amp;id='.$data['offers_id'].'">&raquo;</a></div>';
			}
		...
		echo '<img src="/images/img/open.gif" alt="image" /> <a href="offers.php?act=new">Добавить</a><br />';
	break;
	...
	case 'view':
		...
		if (is_admin(array(101, 102))) {
			echo ' / <a href="/admin/offers.php?act=view&amp;id='.$id.'">Управление</a>';
		}
		echo '<hr />';
		...
		if (!empty($queryoff)) {
			...
			if ($queryoff['offers_status'] <= 1 && $log == $queryoff['offers_user']) {
				echo '<div class="right"><a href="offers.php?act=edit&amp;id='.$id.'">Редактировать</a></div>';
			}
			...
			if ($queryoff['offers_status'] <= 1 && $log != $queryoff['offers_user']) {
				...
				if (empty($queryrated)) {
					echo '<b><a href="offers.php?act=vote&amp;id='.$id.'&amp;uid='.$_SESSION['token'].'"><img src="/images/img/thumb-up.gif" alt="Согласен" /> Согласен</a></b><br />';
				} else {
					echo '<b><a href="offers.php?act=vote&amp;id='.$id.'&amp;uid='.$_SESSION['token'].'"><img src="/images/img/thumb-down.gif" alt="Против" /> Передумал</a></b><br />';
				}
			}

			echo '<a href="offers.php?act=comments&amp;id='.$id.'">Комментарии</a> ('.$queryoff['offers_comments'].') ';
			echo '<a href="offers.php?act=end&amp;id='.$id.'">&raquo;</a></div><br />';
			...
			if (is_user()) {
				if (empty($queryoff['offers_closed'])) {
					echo '<div class="form"><form action="offers.php?act=addcomm&amp;id='.$id.'&amp;uid='.$_SESSION['token'].'" method="post">';
					...
					if ($queryoff['offers_status'] <= 1) {
						if (utf_strlen($title) >= 5 && utf_strlen($title) <= 50) {
							if (utf_strlen($text) >= 5 && utf_strlen($text) <= 1000) {
								...
								redirect("offers.php?act=view&type=$types&id=$id");
							} else {
	...
	case 'comments':
		...
		if (!empty($queryoff)) {
			...
			echo '<img src="/images/img/balloon.gif" alt="image" /> <b><a href="offers.php?act=view&amp;id='.$queryoff['offers_id'].'">'.$queryoff['offers_title'].'</a></b><br /><br />';

			echo '<a href="#down"><img src="/images/img/downs.gif" alt="image" /></a> ';
			echo '<a href="offers.php?type=0">Предложения</a> / ';
			echo '<a href="offers.php?type=1">Проблемы</a> / ';
			echo '<a href="offers.php?act=end&amp;id='.$id.'">Обновить</a><hr />';
			...
			if ($total > 0) {
				...
				if ($is_admin) {
					echo '<form action="offers.php?act=delcomm&amp;id='.$id.'&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'" method="post">';
				}
			...
			if (is_user()) {
				if (empty($queryoff['offers_closed'])) {
					echo '<div class="form">';
					echo '<form action="offers.php?act=addcomm&amp;id='.$id.'&amp;uid='.$_SESSION['token'].'" method="post">';
					echo '<b>Комментарий:</b><br />';
					echo '<textarea cols="25" rows="5" name="msg"></textarea><br />';
			...
			echo '<img src="/images/img/back.gif" alt="image" /> <a href="offers.php?act=view&amp;id='.$queryoff['offers_id'].'">Вернуться</a><br />';
		} else {
			show_error('Ошибка! Данного предложения или проблемы не существует!');
		}
	break;
						...
						if (empty($queryoff['offers_closed'])) {
							if (is_quarantine($log)) {
								if (is_flood($log)) {
									...
									redirect("offers.php?act=end&id=$id");
								} else {
	...
	case 'new':
		...
		if ($udata['users_point'] >= $config['addofferspoint']) {
			echo '<div class="form">';
			echo '<form action="offers.php?act=add&amp;uid='.$_SESSION['token'].'" method="post">';
		...
		if (is_admin()) {
			if ($uid == $_SESSION['token']) {
				if (!empty($del)) {
					...
					redirect("offers.php?act=comments&id=$id&start=$start");
				} else {
