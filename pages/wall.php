...
if (!empty($queryuser)) {
	...
	############################################################################################
		...
		case 'add':
			...
			echo '<img src="/images/img/back.gif" alt="image" /> <a href="wall.php?uz='.$uz.'">Вернуться</a><br />';
		break;
			...
			if (is_user()) {
				if ($uid == $_SESSION['token']) {
					...
					if (!empty($data)) {
						...
						if (empty($queryspam)) {
							if (is_flood($log)) {
								DB::run() -> query("INSERT INTO `spam` (`spam_key`, `spam_idnum`, `spam_user`, `spam_login`, `spam_text`, `spam_time`, `spam_addtime`, `spam_link`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", array(4, $data['wall_id'], $log, $data['wall_login'], $data['wall_text'], $data['wall_time'], SITETIME, $config['home'].'/pages/wall.php?uz='.$uz.'&amp;start='.$start));
