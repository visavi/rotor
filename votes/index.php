	...
	case 'index':
		...
		if (count($votes) > 0) {
			foreach($votes as $valvote) {
				echo '<div class="b">';
				echo '<img src="/images/img/stat.gif" alt="image" /> <b><a href="index.php?act=poll&amp;id='.$valvote['vote_id'].'">'.$valvote['vote_title'].'</a></b></div>';
			...
			if (empty($votes['vote_closed'])) {
				...
				if ($total > 0) {
					...
					if ((is_user() && empty($polls)) && empty($_GET['result'])) {
						...
						echo '<form action="index.php?act=vote&amp;id='.$id.'&amp;uid='.$_SESSION['token'].'" method="post">';
						...
						echo '<img src="/images/img/history.gif" alt="image" /> <a href="index.php?act=poll&amp;id='.$id.'&amp;result=show">Результаты</a><br />';
						...
						if (!empty($_GET['result'])) {
							echo '<img src="/images/img/stat.gif" alt="image" /> <a href="index.php?act=poll&amp;id='.$id.'">К вариантам</a><br />';
						}
						echo '<img src="/images/img/users.gif" alt="image" /> <a href="index.php?act=voters&amp;id='.$id.'">Проголосовавшие</a><br />';
					}
						...
						if (empty($votes['vote_closed'])) {
							...
							if (!empty($queryanswer)) {
								...
								if (empty($polls)) {
									...
									redirect("index.php?act=poll&id=$id");
...
echo '<img src="/images/img/luggage.gif" alt="image" /> <a href="history.php">Архив голосований</a><br />';
