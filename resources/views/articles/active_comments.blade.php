...
@section('title', sprintf('%s - %s (%s)', __('index.blogs'), __('blogs.comments_list', ['user' => $user->getName()]), __('main.page_num', ['page' => $comments->currentPage()])))
        ...
        @foreach ($comments as $comment)
            <div class="section mb-3 shadow">
                <div class="section-title">
                    ...
                    <a href="{{ route('articles.view', ['slug' => $comment->relate->slug, 'cid' => $comment->id]) }}">{{ $comment->title }}</a> <span class="badge bg-adaptive">{{ $comment->count_comments }}</span>
