            ...
            <div class="section mb-3 shadow">
                ...
                <div class="section-user d-flex align-items-start">
                    <div class="flex-grow-1">
                        ...
                        <small class="section-date text-muted fst-italic" data-date="{{ dateFixed($data->created_at, original: true) }}">{{ dateFixed($data->created_at) }}</small><br>
                    ...
                    @if (getUser())
                        <div class="text-end section-action">
                            @if (getUser('id') !== $data->author_id)
                                ...
                                <a href="#" onclick="return sendComplaint(this)" data-type="{{ $data->getMorphClass() }}" data-id="{{ $data->id }}" data-page="{{ $messages->currentPage() }}" rel="nofollow" title="{{ __('main.complain') }}"><i class="fa fa-bell text-muted"></i></a>
                            @endif

                            @if (isAdmin() || getUser('id') === $user->id)
                                <a href="#" onclick="return deleteWall(this)" data-id="{{ $data->id }}" data-login="{{ $data->user->login }}" data-bs-toggle="tooltip" title="{{ __('main.delete') }}"><i class="fa fa-times text-muted"></i></a>
                            @endif
                        </div>
                    @endif
                </div>
