<ul class="list-inline hiding text-left">
    ...
    <li class="list-inline-item" data-toggle="tooltip" title="{{ trans('index.loading_speed') }}"><i class="fa fa-rocket"></i> {{ round(microtime(true) - STARTTIME, 4) }} {{ trans('index.seconds') }}</li>
</ul>

@if (env('APP_DEBUG'))
<pre class="text-left prettyprint linenums pre-scrollable">
@foreach ($queries as $key=> $query)
{{ $query['query'] }} ({{ number_format($query['time'] /1000, 5) }} {{ trans('index.seconds') }})
@endforeach
</pre>
@endif
