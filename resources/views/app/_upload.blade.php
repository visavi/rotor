<?php
$condition = $paste ?? 0;
$onclick = $condition ? 'return pasteImage(this);' : false;
?>
...
<div class="js-images">
    @if ($files->isNotEmpty())
        @foreach ($files as $file)
            <span class="js-image">
                {!! resizeImage($file->hash, ['width' => 100, 'onclick' => $onclick]) !!}
...
<div class="js-image-template d-none">
    <span class="js-image">
        <img src="#" width="100" onclick="{{ $onclick }}" alt="" class="img-fluid">
...
<label class="btn btn-sm btn-secondary" for="image">
    <input id="image" type="file" name="image" onchange="return submitImage(this, {{ $condition }});" data-id="{{ $id or 0 }}" data-type="{{ $type }}" data-token="{{ $_SESSION['token'] }}" hidden>
