<div class="comment-thread" id="comment_{{ $comment->id }}">
    <div class="section mb-2 shadow comment-item" data-id="{{ $comment->id }}">
        ...
        <div class="section-user d-flex align-items-start">
            ...
            <div class="text-end">
                @if ($comment->children->isNotEmpty())
                    <button type="button" class="btn btn-sm btn-link p-0 me-1 comment-collapse-btn" data-id="{{ $comment->id }}" data-bs-toggle="tooltip" title="{{ __('main.collapse') }}">
                ...
                @if (getUser())
                    <div class="section-action">
                        @if (getUser('id') !== $comment->user_id)
                            @if (! ($closed ?? false))
                                <a href="#" onclick="return openReplyForm({{ $comment->id }})" data-bs-toggle="tooltip" title="{{ __('main.reply') }}"><i class="fa fa-reply text-muted"></i></a>
                            @endif

                            <a href="#" onclick="return postQuote(this)" data-bs-toggle="tooltip" title="{{ __('main.quote') }}"><i class="fa fa-quote-right text-muted"></i></a>

                            <a href="#" onclick="return sendComplaint(this)" data-type="{{ $comment->relate->getMorphClass() }}" data-id="{{ $comment->id }}" rel="nofollow" data-bs-toggle="tooltip" title="{{ __('main.complain') }}"><i class="fa fa-bell text-muted"></i></a>
                        @endif
    ...
    @if (getUser() && ! ($closed ?? false) && getUser('id') !== $comment->user_id)
        <div class="reply-form d-none mb-2" id="reply-form-{{ $comment->id }}">
            <form action="{{ $action }}" method="post">
                @csrf
                <input type="hidden" name="parent_id" value="{{ $comment->id }}">
                <div class="mb-2">
                    <textarea class="form-control" name="msg" rows="3" maxlength="{{ setting('comment_text_max') }}" placeholder="{{ __('main.message') }}..." required></textarea>
                </div>
                <button class="btn btn-sm btn-success">{{ __('main.write') }}</button>
                <button type="button" class="btn btn-sm btn-secondary ms-1" onclick="closeReplyForm({{ $comment->id }})">{{ __('main.cancel') }}</button>
            </form>
        </div>
    @endif

    @if ($comment->children->isNotEmpty())
        <div class="comment-children {{ $comment->depth > 0 ? 'ms-3 border-start border-2 ps-2' : 'ms-4' }}" id="comment-children-{{ $comment->id }}">
            @foreach ($comment->children as $child)
                @include('app/_comment_item_simple', ['comment' => $child, 'action' => $action, 'closed' => $closed])
            @endforeach
        </div>
    @endif
</div>
