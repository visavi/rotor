...
@section('header')
    <div class="float-end">
        @if (getUser())
            ...
            @if (isAdmin())
                <a class="btn btn-adaptive" href="{{ route('admin.forums.forum', ['id' => $forum->id, 'page' => $topics->currentPage()]) }}"><i class="fas fa-wrench"></i></a>
            @endif
        @endif
    </div>
        ...
        @foreach ($forum->children as $child)
            <div class="section mb-3 shadow border-start border-info border-5">
                ...
                @if ($child->description)
                    <div class="section-description text-muted fst-italic small">{{ renderText($child->description) }}</div>
                @endif
