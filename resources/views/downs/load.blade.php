...
@section('header')
    <div class="float-end">
        @if (getUser())
            ...
            @if (isAdmin())
                <a class="btn btn-adaptive" href="{{ route('admin.loads.load', ['id' => $category->id, 'page' => $downs->currentPage()]) }}"><i class="fas fa-wrench"></i></a>
            @endif
        @endif
    </div>
        ...
        @foreach ($downs as $data)
            <div class="section mb-3 shadow">
                ...
                <div class="section-content">
                    ...
                    <a href="{{ route('downs.view', ['id' => $data->id]) }}#comments">{{ __('main.comments') }}</a> <span class="badge bg-adaptive">{{ $data->count_comments }}</span>
                </div>
            </div>
        @endforeach
