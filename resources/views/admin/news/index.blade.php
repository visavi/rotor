        ...
        @foreach ($news as $data)
            <div class="section mb-3 shadow">
                ...
                <div class="section-content short-view">
                    @if ($data->getMedia()->isNotEmpty())
                        @include('app/_media_slider', ['model' => $data, 'files' => $data->getMedia()])
                    @endif
                ...
                <i class="fa-regular fa-comment"></i> <a href="{{ route('news.view', ['id' => $data->id]) }}">{{ __('main.comments') }}</a> <span class="badge bg-adaptive">{{ $data->count_comments }}</span>
            </div>
        @endforeach
