<form action="/admin/setting?act=load" method="post">
    ...
    <div class="form-group{{ hasError('sets[downlist]') }}">
        ...
        <input type="text" class="form-control" id="downlist" name="sets[downlist]" maxlength="2" value="{{ getInput('sets[downlist]', $settings['downlist']) }}" required>
        {!! textError('sets[downlist]') !!}
    </div>

    <div class="form-group{{ hasError('sets[downcomm]') }}">
        <label for="downcomm">Комментариев в загрузках:</label>
        <input type="text" class="form-control" id="downcomm" name="sets[downcomm]" maxlength="2" value="{{ getInput('sets[downcomm]', $settings['downcomm']) }}" required>
        {!! textError('sets[downcomm]') !!}
    </div>

    <div class="form-group{{ hasError('sets[ziplist]') }}">
        <label for="ziplist">Просмотр архивов на стр.:</label>
        <input type="text" class="form-control" id="ziplist" name="sets[ziplist]" maxlength="2" value="{{ getInput('sets[ziplist]', $settings['ziplist']) }}" required>
        {!! textError('sets[ziplist]') !!}
    </div>

    <div class="form-group{{ hasError('sets[fileupload]') }}">
        <label for="fileupload">Максимальный вес файла (Mb):</label>
        <input type="text" class="form-control" id="fileupload" name="sets[fileupload]" maxlength="2" value="{{ getInput('sets[fileupload]', round($settings['fileupload'] / 1048576)) }}" required>
    ...
    <div class="form-group{{ hasError('sets[screenupload]') }}">
        ...
        <input type="text" class="form-control" id="screenupload" name="sets[screenupload]" maxlength="2" value="{{ getInput('sets[screenupload]', round($settings['screenupload'] / 1048576)) }}" required>
    ...
    <div class="form-group{{ hasError('sets[screenupsize]') }}">
        ...
        <input type="text" class="form-control" id="screenupsize" name="sets[screenupsize]" maxlength="5" value="{{ getInput('sets[screenupsize]', $settings['screenupsize']) }}" required>
