<form action="/admin/setting?act=guest" method="post">
    ...
    <div class="form-group{{ hasError('sets[sendprivatmailday]') }}">
        ...
        <input type="text" class="form-control" id="lastnews" name="sets[lastnews]" maxlength="2" value="{{ getInput('sets[lastnews]', $settings['lastnews']) }}" required>
        {!! textError('sets[lastnews]') !!}
    </div>

    <div class="form-group{{ hasError('sets[postnews]') }}">
        <label for="postnews">Новостей на страницу:</label>
        <input type="text" class="form-control" id="postnews" name="sets[postnews]" maxlength="2" value="{{ getInput('sets[postnews]', $settings['postnews']) }}" required>
    ...
    <div class="form-group{{ hasError('sets[bookpost]') }}">
        ...
        <input type="text" class="form-control" id="postnews" name="sets[bookpost]" maxlength="2" value="{{ getInput('sets[bookpost]', $settings['bookpost']) }}" required>
    ...
    <div class="form-group{{ hasError('sets[guesttextlength]') }}">
        ...
        <input type="text" class="form-control" id="guesttextlength" name="sets[guesttextlength]" maxlength="5" value="{{ getInput('sets[guesttextlength]', $settings['guesttextlength']) }}" required>
        {!! textError('sets[guesttextlength]') !!}
    </div>

    <div class="form-group{{ hasError('sets[guesttextlength]') }}">
        <label for="chatpost">Сообщений в админ-чате на стр:</label>
        <input type="text" class="form-control" id="chatpost" name="sets[chatpost]" maxlength="2" value="{{ getInput('sets[chatpost]', $settings['chatpost']) }}" required>
