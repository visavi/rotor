...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="/forums/{{ $forum->id  }}?page={{ $page->current }}">{{ __('main.review') }}</a></li>
        </ol>
    </nav>
@stop
            ...
            @foreach ($topics as $topic)
                <div class="b" id="topic_{{ $topic->id }}">
                    ...
                    <div class="float-right">
                        ...
                        <a href="/admin/topics/delete/{{ $topic->id }}?page={{ $page->current }}&amp;token={{ $_SESSION['token'] }}" onclick="return confirm('{{ __('forums.confirm_delete_topic') }}')" title="{{ __('main.delete') }}"><i class="fa fa-times text-muted"></i></a>
                    </div>
