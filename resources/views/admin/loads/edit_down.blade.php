...
@section('content')
    ...
    @if (isAdmin('boss'))
        @if ($down->active)
            ...
            <a class="me-3" href="/admin/downs/publish/{{ $down->id }}?token={{ csrf_token() }}" onclick="return confirm('{{ __('loads.confirm_unpublish_down') }}')">{{ __('main.unpublish') }}</a>
        @else
            <i class="fa fa-pencil-alt"></i>
            <a class="me-3" href="/admin/downs/publish/{{ $down->id }}?token={{ csrf_token() }}" onclick="return confirm('{{ __('loads.confirm_publish_down') }}')">{{ __('main.publish') }}</a>
        @endif

        <i class="fas fa-times"></i> <a class="me-3" href="/admin/downs/delete/{{ $down->id }}?token={{ csrf_token() }}" onclick="return confirm('{{ __('loads.confirm_delete_down') }}')">{{ __('main.delete') }}</a>
        <hr>
    @endif
