    ...
    <div class="section-form mb-3 shadow">
        <form action="{{ route('admin.guestbook.edit', ['id' => $post->id, 'page' => $page]) }}" method="post">
            ...
            <div class="mb-3{{ hasError('msg') }}">
                ...
                <textarea class="form-control tiptap" maxlength="{{ setting('guestbook_text_max') }}" id="msg" rows="5" name="msg" placeholder="{{ __('main.message') }}" data-relate-type="{{ $post->getMorphClass() }}" data-relate-id="{{ $post->id }}" required>{{ getInput('msg', $post->text) }}</textarea>
