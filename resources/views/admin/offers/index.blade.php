...
@section('header')
    <div class="float-end">
        ...
        <a class="btn btn-adaptive" href="{{ route('offers.index', ['type' => $type, 'page' => $offers->currentPage()]) }}"><i class="fas fa-wrench"></i></a>
    </div>
...
@section('content')
    ...
    @if (isAdmin('boss'))
        <form action="{{ route('admin.offers.restatement') }}" method="post">
            @csrf
