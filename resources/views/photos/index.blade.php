...
@section('header')
    <div class="float-end">
        @if (isAdmin() || (getUser() && setting('photos_create')))
            ...
            @if (isAdmin())
                <a class="btn btn-adaptive" href="{{ route('admin.photos.index', ['page' => $photos->currentPage()]) }}"><i class="fas fa-wrench"></i></a>
            @endif
        @endif
    </div>
        ...
        @foreach ($photos as $photo)
            <div class="section mb-3 shadow">
                ...
                <div class="section-content">
                    ...
                    <a href="{{ route('photos.view', ['id' => $photo->id]) }}#comments">{{ __('main.comments') }}</a> <span class="badge bg-adaptive">{{ $photo->count_comments }}</span>
                </div>
            </div>
        @endforeach
