...
@section('title', __('index.loads') . ' - ' . __('loads.active_comments', ['user' => $user->getName()]) . ' (' . __('main.page_num', ['page' => $comments->currentPage()]) . ')')

@section('header')
    <h1>{{ __('loads.active_comments', ['user' => $user->getName()]) }}</h1>
@stop
        ...
        @foreach ($comments as $data)
            <div class="section mb-3 shadow">
                <div class="section-title">
                    ...
                    @if (isAdmin())
                        <div class="float-end">
                            <a href="#" onclick="return deleteComment(this)" data-rid="{{ $data->relate_id }}" data-id="{{ $data->id }}" data-type="{{ $data->relate->getMorphClass() }}" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('main.delete') }}"><i class="fa fa-times text-muted"></i></a>
                        </div>
                    @endif
                </div>
