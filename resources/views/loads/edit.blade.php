        ...
        <form action="/downs/edit/{{ $down->id }}" method="post" enctype="multipart/form-data">
            ...
            @if ($down->getFiles()->isNotEmpty())
                @foreach ($down->getFiles() as $file)
                    ...
                    <b><a href="/uploads/files/{{ $file->hash }}">{{ $file->name }}</a></b> ({{ formatSize($file->size) }}) (<a href="/downs/delete/{{ $down->id }}/{{ $file->id }}" onclick="return confirm('Вы действительно хотите удалить данный файл?')">Удалить</a>)<br>
                @endforeach
            @endif
