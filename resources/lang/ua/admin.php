<?php

return [
    'antimat' => [
        'text' => 'Усі слова зі списку замінятимуться на ***<br>
        Щоб видалити слово, наытисніть на нього, додати слово можна у формі нижче<br>',
        'words'          => 'Список слів',
        'total_words'    => 'Усього слів',
        'confirm_clear'  => 'Ви впевнені, що хочете видалити всі слова?',
        'empty_words'    => 'Слов ще немає!',
        'enter_word'     => 'Введіть слово',
        'not_enter_word' => 'Ви не ввели слово для занесення до списку!',
        'word_listed'    => 'Введене слово вже є у списку!',
    ],

    'banhists' => [
        'history'       => 'Історія',
        'search_user'   => 'Пошук користувача',
        'empty_history' => 'Історії банів ще немає!',
        'view_history'  => 'Перегляд історії',
    ],

    'bans' => [
        'login_hint'    => 'Введіть логін користувача, який необхідно змінити',
        'user_ban'      => 'Бан користувача',
        'change_ban'    => 'Зміна бана',
        'time_ban'      => 'Час бана',
        'banned'        => 'Забанити',
        'ban_hint'      => 'Увага! Постарайтеся якнайдокладніше описати причину бана',
        'confirm_unban' => 'Ви дійсно бажаєте розбанити користувача?',

        'forbidden_ban'      => 'Заборонено банити адміністрацію сайту!',
        'user_banned'        => 'Цей користувач вже заблокований!',
        'user_not_banned'    => 'Цей користувач не забанен!',
        'time_not_indicated' => 'Ви не вказали час бана!',
        'time_not_selected'  => 'Не вибрано час бана!',
        'time_empty'         => 'Занадто маленький час бана!',
        'success_banned'     => 'Користувач успішно забанен!',
        'success_unbanned'   => 'Користувач успішно розбанений!',
    ],

    'blacklists' => [
        'email'      => 'Email',
        'logins'     => 'Логини',
        'domains'    => 'Домени',
        'empty_list' => 'Список ще порожній!',

        'type_not_found' => 'Вказаний тип не знайдено!',
        'invalid_login'  => 'Неприпустимі символи в логіні!',
    ],

    'chat' => [
        'clear'           => 'Очистити чат',
        'confirm_clear'   => 'Ви дійсно хочете очистити адмін-чат?',
        'edit_message'    => 'Редагування повідомлення',
        'success_cleared' => 'Адмін-чат успішно очищений!',
    ],

    'errors' => [
        'hint'            => 'Увага! Запис логів вимкнено в налаштуваннях!',
        'errors'          => 'Помилки',
        'autobans'        => 'Автобани',
        'confirm_clear'   => 'Ви впевнені, що хочете очистити логи?',
        'logs_not_exist'  => 'Зазначені логи не існують!',
        'success_cleared' => 'Логи успішно очищені!',
    ],

    'ipbans' => [
        'history'             => 'Історія автобанів',
        'empty_ip'            => 'У бан-листі поки що порожньо!',
        'confirm_clear'       => 'Ви дійсно хочете очистити список IP?',
        'ip_invalid'          => 'Ви ввели неприпустиму IP-адресу',
        'ip_exists'           => 'Введений IP вже є у списку!',
        'ip_success_added'    => 'IP успішно додано до списку!',
        'ip_selected_deleted' => 'Вибрані IP успішно видалені зі списку!',
        'ip_success_cleared'  => 'Список IP успішно очищений!',
    ],

    'modules' => [
        'module'                       => 'Модуль',
        'migrations'                   => 'Міграції',
        'symlink'                      => 'Сімлінк',
        'publish'                      => 'Публікація файлів',
        'config'                       => 'Налаштування',
        'routes'                       => 'Роути',
        'hooks'                        => 'Хукі',
        'helpers'                      => 'Хелпери',
        'middleware'                   => 'Middleware',
        'empty_modules'                => 'Модулі ще не завантажені!',
        'confirm_delete'               => 'Ви дійсно хочете видалити модуль?',
        'confirm_delete_data'          => 'Будуть видалені таблиці модуля разом з усіма записами та його налаштування. Відновити дані буде неможливо. Продовжити?',
        'hint'                         => 'Увага! При видаленні модуля будуть видалені всі міграції та зміни в БД',
        'module_not_found'             => 'Цей модуль не знайдено!',
        'module_success_installed'     => 'Модуль успішно встановлений!',
        'module_success_updated'       => 'Модуль успішно оновлено!',
        'module_success_enabled'       => 'Модуль успішно включений!',
        'module_success_disabled'      => 'Модуль успішно вимкнено!',
        'load_failed'                  => 'Не вдалося завантажити модуль(і)',
        'load_failed_hint'             => 'Деталі помилки дивіться в логах.',
        'load_error'                   => 'Помилка завантаження',
        'module_success_deleted'       => 'Модуль успішно видалено!',
        'safe_mode_enabled'            => 'Видалення модулів заборонено (MODULES_SAFE_MODE=true)!',
        'delete_files'                 => 'Видалити файли',
        'delete_files_not_uninstalled' => 'Спочатку видаліть модуль із системи!',
        'confirm_delete_files'         => 'Видалити файли модуля з диска? Цю дію неможливо скасувати!',
        'module_files_deleted'         => 'Файли модуля видалено з диска!',
        'marketplace'                  => 'Каталог',
        'updates_available'            => 'Доступні оновлення модулів (:count)',
        'upload'                       => 'Завантажити',
        'download'                     => 'Завантажити',
        'upload_from_file'             => 'Встановити з ZIP-файлу',
        'upload_from_url'              => 'Встановити за URL',
        'zip_file'                     => 'ZIP-архів',
        'zip_structure'                => 'Структура архіву',
        'upload_warning'               => 'Встановлюйте лише модулі з перевірених джерел. Шкідливі модулі можуть зламати ваш сайт.',
        'upload_invalid_file'          => 'Некоректний файл!',
        'upload_success_extracted'     => 'Модуль успішно розпаковано!',
        'update_download'              => 'Завантажити оновлення',
        'update_to'                    => 'Оновити до :version',
        'update_extracted'             => 'Оновлення завантажено. Натисніть «Застосувати оновлення», щоб встановити!',
        'download_invalid_url'         => 'Некоректний URL!',
        'download_failed'              => 'Не вдалося завантажити архів!',
        'download_too_large'           => 'Розмір файлу перевищує :size!',
        'download_not_zip'             => 'Файл за посиланням не є ZIP-архівом!',
        'zip_open_failed'              => 'Не вдалося відкрити ZIP-архів!',
        'zip_invalid_path'             => 'Виявлено небезпечний шлях в архіві!',
        'zip_invalid_structure'        => 'Архів повинен містити один кореневий каталог!',
        'zip_invalid_name'             => 'Некоректна назва модуля в архіві!',
        'zip_no_module_file'           => 'Файл module.php не знайдено в архіві!',
        'zip_backup_failed'            => 'Не вдалося створити резервну копію модуля!',
        'zip_extract_failed'           => 'Не вдалося розпакувати архів!',
        'module_dir_exists'            => 'Директорія модуля вже існує!',
        'incompatible'                 => 'Несумісний',
        'conflict'                     => 'Конфлікт реєстрів',
        'files_missing'                => 'Файли відсутні',
        'requires'                     => 'Потрібна версія рушія',
        'latest_version'               => 'Доступна нова версія',
        'changelog'                    => 'Що нового у версії',
        'changelog_full'               => 'Історія змін',
    ],

    'registries' => [
        'title'                      => 'Реєстри модулів',
        'add_registry'               => 'Додати реєстр',
        'add'                        => 'Додати',
        'source'                     => 'Джерело',
        'updated'                    => 'Оновлено',
        'modules_count'              => 'Модулів',
        'not_fetched'                => 'Не завантажено',
        'empty'                      => 'Реєстри не додані',
        'no_modules'                 => 'Немає доступних модулів. Додайте реєстр.',
        'invalid_url'                => 'Некоректний URL реєстру!',
        'already_exists'             => 'Цей реєстр вже додано!',
        'confirm_delete'             => 'Видалити цей реєстр?',
        'registry_success_added'     => 'Реєстр успішно додано!',
        'registry_success_deleted'   => 'Реєстр успішно видалено!',
        'registry_success_refreshed' => 'Реєстр успішно оновлено!',
        'registry_fetch_failed'      => 'Не вдалося оновити реєстр!',
    ],

    'notices' => [
        'confirm_delete'         => 'Ви дійсно хочете видалити цей шаблон?',
        'empty_notices'          => 'Шаблонів ще немає!',
        'edit'                   => 'Редагування шаблону',
        'edit_system_template'   => 'Ви редагуєте системний шаблон',
        'system_template'        => 'Системний шаблон',
        'create'                 => 'Створення шаблону',
        'notice_invalid'         => 'Неприпустима назва типу шаблону!',
        'notice_length'          => 'Занадто довгий або короткий тип шаблону!',
        'notice_exists'          => 'Цей тип вже є в списку!',
        'notice_success_saved'   => 'Шаблон успішно збережено!',
        'notice_not_found'       => 'Цей шаблон не знайдено!',
        'notice_protect'         => 'Заборонено видаляти захищений шаблон!',
        'notice_success_deleted' => 'Шаблон успішно видалено!',
    ],

    'reglists' => [
        'enabled'                => 'Підтвердження реєстрацій увімкнено!',
        'disabled'               => 'Підтвердження реєстрації вимкнено!',
        'empty_users'            => 'Немає користувачів, які потребують підтвердження реєстрації!',
        'users_not_selected'     => 'Відсутні обрані користувачі!',
        'users_success_approved' => 'Вибрані користувачі успішно схвалені!',
        'users_success_deleted'  => 'Вибрані користувачі успішно видалені!',
    ],

    'rules' => [
        'empty_rules'         => 'Правила сайту ще не встановлені!',
        'editing_rules'       => 'Редагування правил',
        'variables'           => 'Внутрішні змінні',
        'sitename'            => 'Назва сайту',
        'rules_empty'         => 'Ви не ввели текст із правилами сайту!',
        'rules_success_saved' => 'Правила успішно збережені!',
    ],

    'spam' => [
        'go_to_message' => 'Перейти до повідомлення',
        'empty_spam'    => 'Скарг ще немає!',
    ],

    'not_exists_boss' => '
        Увага! Відсутня профіль суперадміну<br>
        У базі даних не знайдено користувача з правами boss',
    'exists_install' => '
        Увага! Необхідно видалити файл app/Http/Controllers/InstallController.php<br>
        Наявність цього файлу може порушити безпеку сайту. Видаліть його прямо зараз!',

    'upgrade' => [
        'site_version'      => 'Версія сайту v:version',
        'new_version'       => 'Доступне оновлення системи!',
        'actual_version'    => 'У вас актуальна версія сайту',
        'download'          => 'Завантажити',
        'pending'           => 'Очікують виконання міграцій: :count',
        'run'               => 'Виконати міграції (:count)',
        'db_actual'         => 'База даних в актуальному стані',
        'running'           => 'Виконується...',
        'done_btn'          => 'Готово',
        'all_done'          => 'Всі міграції виконані',
        'request_error'     => 'Помилка запиту',
        'core_updates'      => 'Оновлення ядра',
        'db_tab'            => 'Міграції БД',
        'core_actual'       => 'Ядро в актуальному стані',
        'major_blocked'     => 'Доступний новий мажорний реліз, але спочатку оновіться до версії :version — перехід через мажор ламає сумісність модулів',
        'modules_outdated'  => 'Ці модулі зібрані під поточний мажор і після оновлення можуть не працювати — оновіть їх одразу після переходу:',
        'perm_error'        => 'Немає прав на запис у директорії: :dirs',
        'update_download'   => 'Завантажити та оновити (:size)',
        'download_full'     => 'Повна версія (:size)',
        'reinstall'         => 'Перевстановити (:size)',
        'reinstall_tab'     => 'Перевстановлення',
        'reinstall_hint'    => 'Завантажити та перевстановити поточну версію — для відновлення змінених або пошкоджених файлів ядра. Версія сайту не змінюється.',
        'reinstall_confirm' => 'Поточна версія буде завантажена та перевстановлена. Продовжити?',
        'update_progress'   => 'Завантажується...',
        'update_applying'   => 'Застосовується...',
        'update_done'       => 'Оновлення застосовано',
        'update_errors'     => 'Не вдалося оновити файли: :files',
        'update_reload'     => 'Оновити сторінку',
        'update_confirm'    => 'Буде завантажено та застосовано оновлення. Продовжити?',
        'invalid_params'    => 'Невірні параметри',
    ],
];
