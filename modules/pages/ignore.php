...
if (is_user()) {
	...
	############################################################################################
		case 'index':
			...
			echo '<br /><div class="form"><form method="post" action="ignore.php?act=add&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'">';
		...
		case 'add':
							...
							if ($total <= $config['limitignore']) {
								...
								if (!is_ignore($log, $uz)){
									...
									if (empty($ignorstr)) {
										...
										$textpriv = '<img src="/images/img/custom.gif" alt="custom" /> Пользователь [b]'.nickname($log).'[/b] добавил вас в свой игнор-лист!';
									...
									redirect("ignore.php?start=$start");
			...
			echo '<img src="/images/img/back.gif" alt="image" /> <a href="ignore.php?start='.$start.'">Вернуться</a><br />';
		break;
		...
		case 'note':
			...
			if ($id > 0) {
				...
				if (!empty($data)) {
					...
					echo '<div class="form">';
					echo '<form method="post" action="ignore.php?act=editnote&amp;id='.$id.'&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'">';
					echo 'Заметка:<br />';
		...
		case 'editnote':
			...
			echo '<img src="/images/img/back.gif" alt="image" /> <a href="ignore.php?act=note&amp;id='.$id.'&amp;start='.$start.'">Вернуться</a><br />';
			echo '<img src="/images/img/reload.gif" alt="image" /> <a href="ignore.php?start='.$start.'">К спискам</a><br />';
		break;
