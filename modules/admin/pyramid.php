...
if (is_admin()) {
    ...
    ############################################################################################
        case 'index':
            ...
            if ($total > 0) {
                echo '<div class="form">';
                echo '<form action="/admin/pyramid?act=delstr&amp;uid='.$_SESSION['token'].'" method="post">';

                foreach ($arradv as $val) {
                    echo '<input type="checkbox" name="del[]" value="'.$val['pyramid_id'].'" /> ';
                    echo '<img src="/images/img/edit.gif" alt="image" /> <b><a href="'.$val['pyramid_link'].'">'.$val['pyramid_name'].'</a></b> ('.profile($val['pyramid_user']).') — <a href="/admin/pyramid?act=edit&amp;id='.$val['pyramid_id'].'&amp;uid='.$_SESSION['token'].'">Изменить</a><br />';
                }
        ...
        case 'edit':
            ...
            if (!empty($pyramid)) {
                ...
                echo '<div class="form">';
                echo '<form action="/admin/pyramid?act=change&amp;id='.$pyramid['pyramid_id'].'&amp;uid='.$_SESSION['token'].'" method="post">';
                echo 'Ссылка:<br />';
            ...
            echo '<i class="fa fa-arrow-circle-left"></i> <a href="/admin/pyramid">Вернуться</a><br />';
        break;
