	...
	############################################################################################
		case 'index':
			...
			if ($total > 0) {
				...
				echo '<form action="rekuser.php?act=del&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'" method="post">';
				...
				while ($data = $queryrek -> fetch()) {
					...
					echo '<a href="rekuser.php?act=edit&amp;id='.$data['rek_id'].'&amp;start='.$start.'">Редактировать</a>';
					echo '</div>';
		...
		case 'edit':
			...
			if (!empty($data)) {
				...
				echo '<div class="form">';
				echo '<form action="rekuser.php?act=change&amp;id='.$id.'&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'" method="post">';
				echo 'Адрес сайта:<br />';
			...
			echo '<img src="/images/img/back.gif" alt="image" /> <a href="rekuser.php?start='.$start.'">Вернуться</a><br />';
		break;
		...
		case 'change':
						...
						if (utf_strlen($name) >= 5 && utf_strlen($name) <= 35) {
							if (preg_match('|^#+[A-f0-9]{6}$|', $color) || empty($color)) {
								...
								if (!empty($data)) {
									...
									redirect("rekuser.php?start=$start");
								} else {
			...
			echo '<img src="/images/img/back.gif" alt="image" /> <a href="rekuser.php?act=edit&amp;id='.$id.'&amp;start='.$start.'">Вернуться</a><br />';
		break;
