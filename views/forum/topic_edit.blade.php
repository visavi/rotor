...
@section('content')
    ...
    <div class="form">
        <form action="/topic/<?=$topic['topics_id']?>/edit" method="post">
            <input type="hidden" name="token" value="{{ $_SESSION['token'] }}">


            <div class="form-group{{ App::hasError('title') }}">
                <label for="inputTitle">Название темы</label>
                <input name="title" type="text" class="form-control" id="inputTitle"  maxlength="50" placeholder="Название темы" value="{{ App::getInput('title', $topic['topics_title']) }}" required>
                {!! App::textError('title') !!}
            </div>
