...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        if (Schema::getColumnType('stickers_categories', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Date::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        $this->addTempColumns('stickers_categories', 'dateTime', ['created_at_dt', 'updated_at_dt']);
        $this->convert('stickers_categories', 'created_at_dt', ['created_at', 'updated_at'], static fn ($r) => [
            'created_at_dt' => $toDt($r->created_at),
            'updated_at_dt' => $toDt($r->updated_at),
        ]);
        Schema::table('stickers_categories', function (Blueprint $table) {
    ...
    public function down(): void
    {
        ...
        $this->addTempColumns('stickers_categories', 'integer', ['created_at_int', 'updated_at_int']);
        $this->convert('stickers_categories', 'created_at_int', ['created_at', 'updated_at'], static fn ($r) => [
