...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        if (Schema::getColumnType('polls', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Date::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        $this->addTempColumns('polls', 'dateTime', ['created_at_dt']);
        $this->convert('polls', 'created_at_dt', ['created_at'], static fn ($r) => ['created_at_dt' => $toDt($r->created_at)]);
        Schema::table('polls', fn (Blueprint $table) => $table->dropColumn('created_at'));
        Schema::table('polls', fn (Blueprint $table) => $table->renameColumn('created_at_dt', 'created_at'));
    }
    ...
    public function down(): void
    {
        ...
        $this->addTempColumns('polls', 'integer', ['created_at_int']);
        $this->convert('polls', 'created_at_int', ['created_at'], static fn ($r) => ['created_at_int' => $toInt($r->created_at)]);
        Schema::table('polls', fn (Blueprint $table) => $table->dropColumn('created_at'));
        Schema::table('polls', fn (Blueprint $table) => $table->renameColumn('created_at_int', 'created_at'));
    }
};
