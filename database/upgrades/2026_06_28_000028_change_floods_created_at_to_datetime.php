...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        if (Schema::getColumnType('floods', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Date::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        Schema::table('floods', fn (Blueprint $table) => $table->dateTime('created_at_dt')->nullable());
        $this->convert('floods', ['created_at'], static fn ($r) => ['created_at_dt' => $toDt($r->created_at)]);
        Schema::table('floods', fn (Blueprint $table) => $table->dropColumn('created_at'));
        Schema::table('floods', fn (Blueprint $table) => $table->renameColumn('created_at_dt', 'created_at'));
    }
    ...
    public function down(): void
    {
        ...
        Schema::table('floods', fn (Blueprint $table) => $table->integer('created_at_int')->nullable());
        $this->convert('floods', ['created_at'], static fn ($r) => ['created_at_int' => $toInt($r->created_at)]);
        Schema::table('floods', fn (Blueprint $table) => $table->dropColumn('created_at'));
        Schema::table('floods', fn (Blueprint $table) => $table->renameColumn('created_at_int', 'created_at'));
    }
};
