...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        if (Schema::getColumnType('spam', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Carbon::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        Schema::table('spam', fn (Blueprint $table) => $table->dateTime('created_at_dt')->nullable());
        $this->convert('spam', ['created_at'], static fn ($r) => ['created_at_dt' => $toDt($r->created_at)]);
        ...
        Schema::table('spam', fn (Blueprint $table) => $table->index('created_at'));
    }
    ...
    public function down(): void
    {
        ...
        Schema::table('spam', fn (Blueprint $table) => $table->integer('created_at_int')->nullable());
        $this->convert('spam', ['created_at'], static fn ($r) => ['created_at_int' => $toInt($r->created_at)]);
        Schema::table('spam', function (Blueprint $table) {
            $table->dropIndex(['created_at']);
            $table->dropColumn('created_at');
        });
