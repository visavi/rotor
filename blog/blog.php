...
case 'changeblog':
	...
	render('includes/back', array('link' => 'blog.php?act=editblog&amp;id='.$id, 'title' => 'Вернуться'));
	render('includes/back', array('link' => 'blog.php?act=view&amp;id='.$id, 'title' => 'К статье', 'icon' => 'reload.gif'));
break;
...
case 'addblog':
							...
							if (!empty($blogs)) {
								if (is_quarantine($log)) {
									if (is_flood($log)) {
										...
										redirect("blog.php?act=view&id=$lastid");
	...
	render('includes/back', array('link' => 'blog.php?act=new&amp;cid='.$cid, 'title' => 'Вернуться'));
break;
...
case 'comments':
	...
	render('includes/back', array('link' => '/blog/blog.php?act=view&amp;id='.$id, 'title' => 'Вернуться'));
...
case 'add':
				...
				if (!empty($queryblog)) {
					if (is_quarantine($log)) {
						if (is_flood($log)) {
							...
							redirect("blog.php?act=end&id=$id");
	...
	render('includes/back', array('link' => 'blog.php?act=comments&amp;id='.$id, 'title' => 'Вернуться'));
break;
	...
	if (is_user()) {
		if ($uid == $_SESSION['token']) {
			...
			if (!empty($data)) {
				...
				if (empty($queryspam)) {
					if (is_flood($log)) {
						DB::run() -> query("INSERT INTO `spam` (`spam_key`, `spam_idnum`, `spam_user`, `spam_login`, `spam_text`, `spam_time`, `spam_addtime`, `spam_link`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", array(6, $data['commblog_id'], $log, $data['commblog_author'], $data['commblog_text'], $data['commblog_time'], SITETIME, $config['home'].'/blog/blog.php?act=comments&amp;id='.$id.'&amp;start='.$start));

						notice('Жалоба успешно отправлена!');
						redirect("blog.php?act=comments&id=$id&start=$start");
...
case 'editpost':
	...
	render('includes/back', array('link' => 'blog.php?act=edit&amp;id='.$id.'&amp;pid='.$pid.'&amp;start='.$start, 'title' => 'Вернуться'));
break;
